...
$(document).ready(function () {
	...
	(function(){
		...
		contentImages.each(function() {
		  var obj = this;
		  if($(obj).attr("align") && $(obj).parents(".img").length == 0){
		    $(obj).removeAttr("align")
		    //console.log(obj)
		    var str = "<div class=\"img\" id=\"changeTheId-wrap-img-"+$(obj)[0].id.slice(16)+"\"><a title=\"点击查看原图\">"+$(obj)[0].outerHTML+"</a><strong>"+$(obj)[0].title+"</strong></div>"
		    findInsert(obj,str)
		    $(obj).remove()
		  }
		})
		$(window).bind("scroll",function(e){
			...
			$("#custom_zy, .xinxi,#custom_ciyun , #custom_tuce , #custom_timeline , #custom_qiyeInfo, .related, .h2_line, #content h3").each(function(){
				var obj = $(this);
				if($(this).offset().top < top_distance+screen_height){
					...
					if(Math.abs($(this).offset().top-top_distance) <= nearestValue && $(this).offset().top-top_distance != 0){
						nearestValue = Math.abs($(this).offset().top-top_distance);
						if(obj[0].nodeName != "H3"){
							nearestId = $(this).attr("id")
						}else{
							var parentId,selfIndex = 0;
							parentId = obj.parent().attr("id") != undefined ? obj.parent().prevAll(".h2_line").eq(0).attr("id") : obj.parent().parent().prevAll(".h2_line").eq(0).attr("id")
							obj.parent().find("h3").each(function(i){
								if($(this).html() == obj.html()){selfIndex = i}
							})
							nearestId = [parentId,selfIndex]
						}
					}
				}
			});
			if(nearestId != ""){
				$('#list-catalogue li[catalogkey]').removeClass('current');
				if(typeof(nearestId) == "string"){
					$('#list-catalogue li[catalogkey='+nearestId+']').addClass('current');
				}else{
					$("#list-catalogue li[catalogkey="+nearestId[0]+"]").nextAll().eq(nearestId[1]).addClass("current");
				}
			}
		});
...
function innerLink(title){
	...
	if(mydomain.indexOf("byte.baike.com")!=-1){
		var tmptitle = window.location.href.split('wiki/')[1].split('&fr')[0];
		window.location.href = window.location.href.replace(tmptitle,title);
	}else{
		$.ajax({
